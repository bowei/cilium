...
(Differences from original: IP addresses, add a different cluster-cidr and node cidr mask
size)
...
```
wget https://storage.googleapis.com/kubernetes-release/release/v1.5.0/bin/linux/amd64/kube-apiserver
```
```
wget https://storage.googleapis.com/kubernetes-release/release/v1.5.0/bin/linux/amd64/kube-controller-manager
```
```
wget https://storage.googleapis.com/kubernetes-release/release/v1.5.0/bin/linux/amd64/kube-scheduler
```
```
...
```
wget https://raw.githubusercontent.com/cilium/cilium/master/examples/gce/token.csv
```
...
```
wget https://raw.githubusercontent.com/cilium/cilium/master/examples/gce/authorization-policy.jsonl
```
...
ExecStart=/usr/bin/kube-apiserver \
  ...
  --authorization-policy-file=/var/lcat > ./etcd-config.yml <<EOF
---
endpoints:
...
EOFib/kubernetes/authorization-policy.jsonl \
  --bind-address=0.0.0.0 \
  --enable-swagger-ui=true \
  --etcd-cafile=/var/lib/kubernetes/ca.pem \
  --insecure-bind-address=0.0.0.0 \
  --kubelet-certificate-authority=/var/lib/kubernetes/ca.pem \
  --etcd-servers=https://172.30.0.10:2379,https://172.30.0.11:2379,https://172.30.0.12:2379 \
  --service-account-key-file=/var/lib/kubernetes/kubernetes-key.pem \
  --service-cluster-ip-range=172.26.0.0/24 \
...
You probably have noticed, the IP `172.26.0.1` was previously used when we were creating the
certificates, that IP will be assigned to the kube-apiserver. It's the first IP of the IP
range that we set in `--service-cluster-ip-range`.
